	...
	do {
		...
		if (keypressed == 0) {
			...
			if (timerC(&cursorTime) == TRUE) {
				...
				case 1:
					...
					write_ch(aux, posCursor, wherey,
						 displayChar, backcolor,
						 textcolor, raw);
					//update_screen(aux);
					if (raw != TRUE)
						dump_screen(aux);
		...
		if (keypressed == 1) {
			...
			if (charCount < displayLength) {
				if (ch > 31 && ch < 127) {
					write_ch(aux, positionx + 1, wherey, ch,
						 backcolor, textcolor, raw);
					text[charCount] = ch;
					positionx++;
					charCount++;
		...
		if (ch == K_BACKSPACE) {
			if (positionx > 0 && charCount > 0) {
				...
				charCount--;
				text[charCount] = '\0';
				write_ch(aux, positionx + 1, wherey, '.',
					 backcolor, textcolor, raw);
				if (positionx < limitCursor - 2)
					write_ch(aux, positionx + 2, wherey,
						 '.', backcolor, textcolor,
						 raw);
				//update_screen(aux);
				if (raw != TRUE)
 ...
 */
	...
	if (shadow == 1) {
		for (j = y1 + 1; j <= y2 + 1; j++)
			for (i = x1 + 1; i <= x2 + 1; i++) {
				...
				if ((ch == '\0') || (ch == UNICODEBAR1))
					ch = 0x20;
				write_ch(screen1, i, j, ch, B_BLACK, F_WHITE,
					 0);
			}
	}
	//window
	...
	if (title == 1) {
		for (i = x1; i <= x2; i++)
			write_ch(screen1, i, y1 - 1, ' ', titlecolor,
				 titlecolor, 0);
	}
	// dump_screen(screen1);
}
